...
public interface TradeOfferFactoryType<P extends TradeOffers.Factory> {
    Codec<TradeOffers.Factory> CODEC = BetterTradingMod.TRADE_OFFER_FACTORY_REGISTRY.getCodec().dispatch("type", factory -> ((TradeOfferFactoryTypeHolder) factory).getType(), TradeOfferFactoryType::codec);
    ...
    TradeOfferFactoryType<SellItemForItemsOfferFactory> SELL_ITEM_FOR_ITEMS = register(BetterTradingMod.id("sell_item_for_items"), SellItemForItemsOfferFactory.CODEC);
    TradeOfferFactoryType<TypeAwareSellItemForItemsOfferFactory> TYPE_AWARE_SELL_ITEMS_FOR_ITEM = register(BetterTradingMod.id("type_aware_sell_item_for_items"), TypeAwareSellItemForItemsOfferFactory.CODEC);
    TradeOfferFactoryType<TypeAwareTradeOfferFactory> TYPE_AWARE = register(BetterTradingMod.id("type_aware"), TypeAwareTradeOfferFactory.CODEC);

    TradeOfferFactoryType<EnchantSpecificBookFactory> ENCHANT_SPECIFIC_BOOK = register(BetterTradingMod.id("enchant_specific_book"), EnchantSpecificBookFactory.CODEC);
    TradeOfferFactoryType<SellSpecificPotionHoldingItemFactory> SELL_SPECIFIC_POTION_HOLDING_ITEM = register(BetterTradingMod.id("sell_specific_potion_holding_item"), SellSpecificPotionHoldingItemFactory.CODEC);

    Codec<P> codec();

    static <P extends TradeOffers.Factory> TradeOfferFactoryType<P> register(Identifier id, Codec<P> codec) {
        return Registry.register(BetterTradingMod.TRADE_OFFER_FACTORY_REGISTRY, id, () -> codec);
    }
