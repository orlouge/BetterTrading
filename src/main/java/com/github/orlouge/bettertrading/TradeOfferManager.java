...
public class TradeOfferManager extends JsonDataLoader implements IdentifiableResourceReloadListener {
    ...
    private static final Identifier ID = BetterTradingMod.id("trade_offers");
    ...
    protected void apply(Map<Identifier, JsonElement> prepared, ResourceManager manager, Profiler profiler) {
        ...
        manager.findResources(ID.getPath(), id -> id.getPath().endsWith(".json")).forEach((id, res) -> {
            ...
            if (jsonElement != null) {
                BetterTradingMod.LOGGER.info("Loading {}" + identifier.toString());
                VillagerTrades trades = VillagerTrades.CODEC.decode(JsonOps.INSTANCE, jsonElement).getOrThrow(false, s -> BetterTradingMod.LOGGER.error("Failed to read file {}: {}", identifier.toString(), s)).getFirst();
                if (trades.replace) {
                ...
                */
                ...
                groups.keySet().forEach(k -> BetterTradingMod.LOGGER.info("{} before: {}", identifier.toString(), k));
                trades.offers().forEach((name, group) -> {
                    groups.computeIfPresent(name, (_name, oldGroup) -> TradeGroup.merge(oldGroup, group));
                    groups.putIfAbsent(name, group);
                });
                groups.keySet().forEach(k -> BetterTradingMod.LOGGER.info("{} after: {}", identifier.toString(), k));
        ...
        BetterTradingMod.LOGGER.info("Loaded {} trade offer files", loadedCount.get());
    }
            ...
            tradeMap.keySet().stream().sorted().map(level -> {
                ...
                for (int i = 0; i < trades.length; i++) {
                    ...
                    offers.add(new ExtendedTradeOffer.Factory(
                            trades[i], level, attributes, Optional.empty()
                    ));
                }
